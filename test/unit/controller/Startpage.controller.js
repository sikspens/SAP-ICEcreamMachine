...
sap.ui.define([
	...
	QUnit.test("FloatInstance is correctly created", function(assert) {
		var oSpy = sinon.spy(sap.ui.core.format.NumberFormat, "getFloatInstance");
		var nResult = this.oController.formatNumber("1200000");
		assert.ok(oSpy.calledWith({
			style: "short",
			decimals: 1
		}), "Float instance correctly created");
		assert.equal(nResult, "1.2M", "Number formatted as expected");
		oSpy.restore();
	});

	QUnit.test("Number should be formatted to '1.2M'", function(assert) {
		assert.equal(this.oController.formatNumber("1200000"), "1.2M", "Number formatted as expected");
	});
